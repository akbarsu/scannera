...
commands=(
    ...
    "az network vnet-gateway list --query \"[].{Name:name, VPNType:vpnType, GatewayType:gatewayType, IPConfigurations:ipConfigurations}\" --output json"
    "az network network-intent-policy list --query \"[].{Name:name, PolicySettings:policySettings, Rules:rules}\" --output json"
    ...
    "az network vpn-connection list --query \"[].{Name:name, ConnectionStatus:connectionStatus, SharedKey:sharedKey}\" --output json"
    ...
    "az network local-gateway list --query \"[].{Name:name, Properties:properties, IPConfigurations:ipConfigurations}\" --output json"
    "az network manager list --query \"[].{Name:name, Properties:properties, NetworkGroups:networkGroups}\" --output json"
    "az vm extension list --query \"[].{Name:name, Properties:properties, Settings:settings}\" --output json"
    "az network watcher connection-monitor list --query \"[].{Name:name, Properties:properties, Endpoints:endpoints}\" --output json"
    ...
    "az cdn waf policy list --query \"[].{Name:name, PolicySettings:policySettings, Rules:rules}\" --output json"
    "az ts version list --query \"[].{Name:name, Properties:properties, Versions:versions}\" --output json"
    ...
    "az network front-door waf-policy list --query \"[].{Name:name, PolicySettings:policySettings, Rules:rules}\" --output json"
    "az search service list --query \"[].{Name:name, Properties:properties, QueryKeys:queryKeys}\" --output json"
    ...
    "az webapp deployment slot list --query \"[].{Name:name, Properties:properties, Configurations:configurations}\" --output json"
